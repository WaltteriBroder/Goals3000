...
public interface SingleGoalRepository extends CrudRepository<SingleGoal, Integer> {
    ...
    @Query(value = "SELECT * FROM single_goal WHERE user_id = ?1", nativeQuery = true)
    List<SingleGoal> getSingleGoalsByUser(Integer userId);
    ...
    @Query(value = "SELECT * FROM single_goal u WHERE user_id = ?1 AND added = ?2 AND goal_name =?3",
            nativeQuery = true)
